...
int main (int argc, char **argv){
	...
	while (ros::ok()){
        ...
        if(count>=5){
            eta_h_sum_f = LPF(eta_h_sum_f, eta_h_sum, CO_FRQ_ETA, S_TIME);
            if(eta_h_sum_f>=1.2){
                // ---------------------------- //
                ...
                pose_o_ex_t = 0;
                pose_o_ey_t = 0;
                pose_o_ez_t = pose_o_ez_c + P_ETA_GAIN*eta_h_sum_f;
                ...
                pose_o_ex_t = 0;
                pose_o_ey_t = 0;
                pose_o_ez_t = pose_o_ez_c;//pose_o_ez_c + OF_rl_diff;
                pose_p_x_t = pose_p_x_c + D_SET*cos(pose_o_ez_t);
                pose_p_y_t = pose_p_y_c + D_SET*sin(pose_o_ez_t);
                pose_p_z_t = 1.5;
            }
		...
		file_eta_h_data << count << ", " << eta_h_sum << ", " << eta_h_sum_f << endl;
