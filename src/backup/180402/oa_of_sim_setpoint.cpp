...
int main (int argc, char **argv){
	...
	while (ros::ok()){
        ...
        if(count>=5){
            ...
            sigmoid3 = 1/(1+exp(SIGMOID_P2*(eta_h_sum_f-2)));
            ...
            pose_o_ex_t = 0;
            pose_o_ey_t = 0;
            pose_o_ez_t = pose_o_ez_c + (sigmoid1 * eta_h_ctrl_signed_input) + (sigmoid2 * of_rl_ctrl_input) + (sigmoid3 * s_oz_ctrl_sat_input);
